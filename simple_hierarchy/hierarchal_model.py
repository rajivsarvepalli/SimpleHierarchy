...
class HierarchalModel(nn.Module):
    ...
    Args:
        hierarchy (Dict[Tuple, List[Tuple]): A defined hierarchy through a
        ...
        size (Tuple[int, int, int], optional): A tuple of the (ouput size of
            the base_model or model[len(model) - k - 1], input size of
            model[len(model) - k], output size of model[len(model) - 1]
        output_order ( Optional[List]): The output order of the classes
            returned by forward by their tupled keys in the hierarchy
            dictionary.
        base_model (Optional[nn.Module]): A torch network that will serve as
            the base model.
        model (Optional[nn.ModuleList]): A torch module list that will be
        ...
        k (Optional[int]): A integer representing the number of last layers
            that are distinct.
        dim_to_concat (Optional[int]): The dimension to combine parent ouput
            and the base model's ouput. Typically this is 1.
        feed_from (Optional[int]): The index to feed outputs from parent
            classes to their children. If not given, the last layer is used.
        join_layers (Optional[Dict[Tuple, nn.ModuleList]): A dictionary with
    ...
    def __init__(
        ...
        for node in self.tree:
            ...
            if join_layers:
                ...
                if len(j_layers) != 2:
                    raise ValueError(
                        "Each join_layers entry must have exactly stwo layers."
                    )
